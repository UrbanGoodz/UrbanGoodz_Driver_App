...
class OrderTrackingScreenState extends State<OrderTrackingScreen>
  ...
  void _loadData() async {
    ...
    final realtimeEnabled =
        Get.find<SplashController>().configModel?.websocketEnabled == true;
    final subscribed = realtimeEnabled && await _trackWithPusher();
    if (!subscribed) _timerTrackOrder();
  }

  Future<bool> _trackWithPusher() async {
    bool canTrackDeliveryman =
        Get.find<OrderController>().trackModel?.orderStatus != 'delivered' &&
    ...
    final customerId = profile?.id ?? 0;
    final token = Get.find<AuthController>().getUserToken();
    if (customerId <= 0 || token.isEmpty) return false;

    final subscribed = await PusherHelper.subscribeShopperAccount(
      customerId: customerId,
      bearerToken: token,
      onOrderUpdated: (payload) {
        if (payload['order_id']?.toString() == widget.orderID) {
    ...
    // Driver coordinates are no longer consumed from a public channel.
    // Poll the authenticated order endpoint for location between private
    // account-level status events.
    _timerTrackOrder();
    return subscribed;
  }
